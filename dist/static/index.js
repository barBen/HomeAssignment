...
window.addEventListener("load", function () {
    ...
    webSocket.onmessage = function (event) {
        ...
        if (type === "Point") {
            currentPoint = Object.assign(Object.assign({}, shape), { Height: 0.01, Width: 0.01 });
        }
        if (type === "Rectangle") {
...
function draw(ctx) {
    ...
    ctx.fillRect(currentRectangle.X * 100, currentRectangle.Y * 100, currentRectangle.Width * 100, currentRectangle.Height * 100);
    ctx.fillStyle = "black";
    ctx.fillRect(currentPoint.X * 100, currentPoint.Y * 100, currentPoint.Width * 100, currentPoint.Height * 100);
    const header = document.getElementById("header");
    header.innerHTML = `Point timestamp: ${currentPoint.ts} | Rectangle timestamp: ${currentRectangle.ts}`;
}
function isPointInRect() {
    if (currentRectangle.X < currentPoint.X &&
